...
if (isset($_GET['act']) && $_GET['act'] != '') {
    ...
    switch ($act) {
        ...
        case 'addtk':
            ...
            if (isset($_POST['submit'])) {
                ...
                if (empty(trim($matkhau))) {
                    ...
                    if (!preg_match("/^(?=.*[0-9])(?=.*[A-Z])\w{8,18}$/", $matkhau)) {
                        $check = false;
        ...
        case 'qlbanner':
            ...
            $listbanner = load_all_banner($kyw);
        ...
        case 'addbanner':
            ...
            if (isset($_POST['submit'])) {
                ...
                if (empty($image)) {
                    ...
                    $file_tmp = $_FILES['image']['tmp_name'];
                    $target_file = '../uploads/banner/' . $image;
                    $extension = pathinfo($target_file, PATHINFO_EXTENSION);
                    if (!in_array($extension, ['png', 'jpeg', 'jpg', 'webp'])) {
                        $check = false;
                        $imageErr = 'File không đúng định dạng !';
                ...
                if ($check) {
                    insert_banner($idsanpham, $image, date('Y-m-d'));
                }
            }
            $listsp = load_all_sp('', 0);
        ...
        case 'updatebanner':
            if (isset($_GET['id']) && $_GET['id'] != '') {
                ...
                if ($banner) {
                    ...
                    $oldImage = $banner['imagebanner'];
                    $idsanpham = $banner['idsanpham'];
                }
            }
            $imageErr = '';
            if (isset($_POST['submit'])) {
                ...
                if ($check) {
                    update_banner($idbanner, $image, $oldImage, $idsanpham);
                    echo '<script>
